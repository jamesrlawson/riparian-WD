...
#pool <- subset(dataAvailability.allP, plotID == 1)
sp <- dataAvailability.allP$species
trait <- dataAvailability.allP$allWD
pool <- data.frame(sp, trait)

plot1 <- as.character(subset(dataAvailability.allP, plotID == 1)$species)
abundance1 <- subset(dataAvailability.allP, plotID == 1)$speciescover

summary_verbose1<- test_trait_data(plot1, pool, log=FALSE, reps=999, abweight=TRUE, abdata=abundance1, verbose=TRUE)
